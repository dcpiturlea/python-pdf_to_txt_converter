...
class UI(QMainWindow):
    ...
    def clickedBtn_Import_pdf(self):
        ...
        try:
            ...
            if no_pages < 12:
                self.label_no_page.setText(
                    "Pagini de procesat: " + str(no_pages) + "\n" + "Timp asteptare: " + str(round(
                        no_pages * 3, 2)) + " secunde")
            elif no_pages < 100:
                self.label_no_page.setText(
                    "Pagini de procesat: " + str(no_pages) + "\n" + "Timp asteptare: " + str(round(
                        no_pages * 4.5 / 60, 2)) + " min")
            else:
                self.label_no_page.setText(
                    "Pagini de procesat: " + str(no_pages) + "\n" + "Timp asteptare: " + str(round(
                        no_pages * 5.5 / 60, 2)) + " min")

            self.label_no_page.repaint()

            print("Pagini de procesat: " + str(no_pages))

            self.gui_convert(PDF_file_path, file_types, no_pages)
        except Exception as ex:
            print("A aparut o eroare: " + str(ex))

    def gui_convert(self, PDF_file_path, file_types, no_pages):
        try:
            global txt_file_path
            ...
            if not os.path.isfile(txt_file_path):
                ...
                diff_in_seconds = time_diff.days * 24 * 60 * 60 + time_diff.seconds

                if diff_in_seconds < 60:
                    self.label_no_page.setText("Pagini procesate: " + str(no_pages) + "\n" + "Timp asteptare: " + str(
                        diff_in_seconds) + " secunde")
                else:
                    self.label_no_page.setText("Pagini procesate: " + str(no_pages) + "\n" + "Timp asteptare: " + str(
                        round(diff_in_seconds / 60, 2)) + " min")
